...
//func createClient(req model.Request) []mqtt.Client {
//	if req.Host == "" {
//		panic("参数错误：host")
//	}
//	if req.ClientNum == 0 {
...
//		idx := fmt.Sprintf("%d", nano)
//		opts := mqtt.NewClientOptions().AddBroker(req.Host)
//		opts.SetClientID("mss_tool" + idx)
//		opts.SetUsername("emqx" + idx)
//		opts.SetPassword("public" + idx)
//		opts.SetDefaultPublishHandler(func(c mqtt.Client, message mqtt.Message) {
//			fmt.Println("PublishHandler", message)
...
//	size := req.Burst
//	limiter := rate.NewLimiter(rate.Limit(req.Rate), size)
//	clear(limiter, size)
//	ctx := context.Background()
//
//	StartTime = time.Now()
//	clients := createClient(req)
//	fmt.Printf("客户端数量：%d \n", len(clients))
//	LogCount(true)
//	// 协程生产数据
//	idx := 0
...
//			msgNow := time.Now()
//			// 生产消息
//			msg := fmt.Sprintf(`{"header":{"messageId":"f612fb49845bee6191ea05e1548aa7a2","namespace":"Appliance.Control.ToggleX","triggerSrc":"CloudAlexa","method":"GETACK","payloadVersion":1,"from":"/appliance/2201208098807451860148e1e986b2fb/publish","uuid":"2201208098807451860148e1e986b2fb","timestamp":1673925167,"timestampMs":749,"sign":"2e4375b4631d573499dd0b0585cee295"},"payload":{"channel":0,"togglex":{"channel":0,"onoff":1,"lmTime":%d}},"hellcox-test":"%d-%d-%d"}`, msgNow.Unix(), idx, msgNow.UnixNano(), isSpecial)
//			_ = msg
//			result, _ := rand.Int(rand.Reader, big.NewInt(int64(len(clients))))
//			token := clients[result.Int64()].Publish(req.Topic, 0, false, msg)
//			token.Wait()
//
//			atomic.AddInt64(&ProducerCount, 1)
//			EndTime = time.Now()
//			if req.Number > 0 && ProducerCount >= int64(req.Number) {
//				IsStop = true
//			}
//		}()
//	}
//}
//
//func LogCount(isForeach bool) {
//	if !isForeach {
//		fmt.Println(fmt.Sprintf("[%s]总数:%d, 当前耗时%v", time.Now().Format("2006-01-02 15:04:05"), ProducerCount, EndTime.Sub(StartTime)))
//		return
//	}
//
//	go func() {
...
//			fmt.Println(fmt.Sprintf("[%s]总数:%d, 当前 %d条/秒, 总耗时:%v , 平均%d条/秒",
//				time.Now().Format("2006-01-02 15:04:05"),
//				Now,
//				Now-ProducerCountOri,
//				EndTime.Sub(StartTime),
//				int(math.Ceil(float64(Now/(EndSecond-StartSecond)))),
//			))
//			ProducerCountOri = Now
//			if IsStop {
//				break
//			}
//		}
...
//}
//
//func clear(limiter *rate.Limiter, num int) {
//	//先消耗掉桶内令牌
//	ctx := context.Background()
//	i := 0
//	for true {
